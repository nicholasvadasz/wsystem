...
static const char *normalsTipGeometryShaderSourceCore =
    ...
    "       axis.z = 0.f;\n"
    "       gl_Position = P + N * NORMAL_LINE_LENGTH + axis * "
    "NORMAL_ARROW_WIDTH;\n"
    "       EmitVertex();\n"
    "       gl_Position = P + N * NORMAL_LINE_LENGTH - axis * "
    "NORMAL_ARROW_WIDTH;\n"
...
void GLWidget::initializeGL() {
  ...
  glClearColor(103 / 255.f, 142 / 255.f, 166 / 255.f,
               1); // set the background color

  // Create Shapes shader program
  m_program = new QOpenGLShaderProgram; // allow OpenGL shader programs to be
                                        // linked and used
  ...
  m_program->setUniformValue(m_default_lightPos,
                             QVector3D(70, 70, 70)); // light stuff
  m_program->release();

  // Create Wireframe shader program
  m_wireframeProgram = new QOpenGLShaderProgram;
  m_wireframeProgram->addShaderFromSourceCode(QOpenGLShader::Vertex,
                                              wireframeVertexShaderSourceCore);
  m_wireframeProgram->addShaderFromSourceCode(
      QOpenGLShader::Fragment, wireframeFragmentShaderSourceCore);
  ...
  m_normalsProgram->addShaderFromSourceCode(QOpenGLShader::Vertex,
                                            normalsVertexShaderSourceCore);
  m_normalsProgram->addShaderFromSourceCode(QOpenGLShader::Geometry,
                                            normalsGeometryShaderSourceCore);
  m_normalsProgram->addShaderFromSourceCode(QOpenGLShader::Fragment,
                                            normalsFragmentShaderSourceCore);
  ...
  m_normalsTipsProgram->addShaderFromSourceCode(
      QOpenGLShader::Vertex, normalsTipVertexShaderSourceCore);
  m_normalsTipsProgram->addShaderFromSourceCode(
      QOpenGLShader::Geometry, normalsTipGeometryShaderSourceCore);
  m_normalsTipsProgram->addShaderFromSourceCode(
      QOpenGLShader::Fragment, normalsTipFragmentShaderSourceCore);
  ...
  // m_camera is the model-view matrix. The projection matrix is separately
  // tracked as m_proj
  m_camera = glm::translate(
      m_camera,
      glm::vec3(0, 0, -4 + m_zoomZ)); // Camera stuff (facing -z direction)
}
...
void GLWidget::paintGL() {
  ...
  if (settings.showWireframeNormals) {
    ...
    m_normalsTipsProgram->setUniformValue(m_normalTip_projLoc,
                                          glmMatToQMat(m_proj));
    m_normalsTipsProgram->setUniformValue(m_normalTip_mvLoc,
                                          glmMatToQMat(m_camera * m_world));
    glDrawArrays(GL_TRIANGLES, 0, m_numTriangles);

    m_normalsProgram->bind(); // arrow body
    m_normalsProgram->setUniformValue(m_normal_projLoc, glmMatToQMat(m_proj));
    m_normalsProgram->setUniformValue(m_normal_mvLoc,
                                      glmMatToQMat(m_camera * m_world));
    ...
    glPolygonOffset(-1, -1);
    m_wireframeProgram->bind();
    m_wireframeProgram->setUniformValue(m_wireframe_projLoc,
                                        glmMatToQMat(m_proj));
    m_wireframeProgram->setUniformValue(m_normal_mvLoc,
                                        glmMatToQMat(m_camera * m_world));
    glDrawArrays(GL_TRIANGLES, 0, m_numTriangles);
    glPolygonOffset(0, 0);
  }
...
QMatrix4x4 GLWidget::glmMatToQMat(glm::mat4x4 m) {
  ...
  QMatrix4x4 qMat(m[0][0], m[1][0], m[2][0], m[3][0], m[0][1], m[1][1], m[2][1],
...
void GLWidget::updateView() {
  m_camera = glm::translate(glm::vec3(0.0, 0.0, -4.0 + m_zoomZ)) *
